export default function ItemCard({ item, onDelete, showDelete = true }) {
  const name = item.ai_name || item.name || "Desconhecido";
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="p-3">
        <p className="text-xs text-indigo-600 font-semibold uppercase tracking-wide">{series}</p>
        <h3 className="font-bold text-gray-800 leading-tight mt-0.5">{name}</h3>
        ...
        <div className="flex flex-wrap gap-1 mt-2">
          ...
          {item.owned === true && (
            <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">✓ tenho</span>
          )}
          {item.owned === false && (
            <span className="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded-full">✗ falta</span>
          )}
        </div>
        ...
        {showDelete && onDelete && (
          <button
            ...
            className="mt-3 w-full text-xs text-red-500 hover:text-red-700 hover:bg-red-50 py-1 rounded transition-colors"
          >
            Remover
          </button>
        )}
